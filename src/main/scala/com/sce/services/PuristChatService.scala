...
object PuristChatService extends PuristJsonSupport with NlpJsonSupport {
  ...
  def processMsgToNlp(puristMsgObj: PuristReqModelObj) = {
    ...
    try {
      ...
      if (puristMsgObj.file_url.nonEmpty) {

        var msgDtls = new MessageDtlsObj("", "ATCMT", Nil, "")

        msgDtls.attachments = List(new NLPCommonAtcmtObj(PURIST_USER_DOMAIN_URL + puristMsgObj.file_url.getOrElse(""), puristMsgObj.body.getOrElse("")))
      ...
      } else if (puristMsgObj.body.nonEmpty) {
