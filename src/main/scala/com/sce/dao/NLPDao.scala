...
import scala.slick.jdbc.JdbcBackend.SessionDef
import com.sce.exception.IntentNodesConfException
...
object NLPDao extends NlpJsonSupport with DomainComponent with Profile {
        ...
        if (convExist.nonEmpty) {
          ...
          if (keyWordSet.nonEmpty) {
            ...
            if (languagesfoundinKeywords.length > 1) {
              userLanguageDao.raiseLocaleConfirmation(languagesfoundinKeywords.mkString("~~~~"), msgEvent, sessionID)
            } else {
              ...
              userMsgForKwdIdentification(msgEvent, sessionID, Some(keyWordSet))
            }
          } else {
        ...
        confrimationCacheRecord.PointerDesc.get match {
          ...
          case LANGUAGE =>
            ...
            if (languagesfound.contains(msgEvent.msgDtls.msgTxtWithoutPunc)) {
              val userSelectedLang = msgEvent.msgDtls.msgTxtWithoutPunc
              logger.info("userSelectedLang: {}", userSelectedLang)
              userLanguageDao.setUserPreferredLocale(msgEvent.platformDtls.userID, userSelectedLang, msgEvent.platformDtls.channelID)
              msgEvent.platformDtls.userLang = userSelectedLang
              conversationPointerTbl.filter(x => x.SessionID === sessionID).delete
              msgEvent.msgDtls.msgTxtWithoutPunc = keywordString
              msgEvent.msgDtls.messageTxt = keywordString
              
              userMsgForKwdIdentification(msgEvent,  sessionID, None)

            } else {
              //if user didn't selected any langauge from confirmation then we have to reload confirmation

              msgEvent.msgDtls.msgTxtWithoutPunc = keywordString
              msgEvent.msgDtls.messageTxt = keywordString

              val keyWordSet = intentIdentificationDao.findKeywordsInSntnce(sessionID, keywordString, true, true, Set.empty)
              val languagesfoundinKeywords = userLanguageDao.getLocales(keyWordSet.mkString(" "))
  ...
  def convFromIntentChoice(msgEvent: NlpReqCommonObj, sessionID: String)(implicit session: JdbcBackend#SessionDef) = { // = db withSession { implicit session =>
    ...
    var userLang = msgEvent.platformDtls.userLang
    try {
      //here we are using oldMessageEvent for sending cached message to keyword identification. Here for the variable msgEvent.msgDtls.msgTxtWithoutPunc is expected to be an intent name for incoming message
      //if user selects choice options we are processing oldMessageEvent
      //here we are using newMessageEvent for sending incoming message to keyword identification. Here for the variable msgEvent.msgDtls.msgTxtWithoutPunc is expected to be an keyword
      //if user didn't selects choice options we are processing newMessageEvent
      val payloadOrUserMsg = msgEvent.msgDtls.msgTxtWithoutPunc
      val newMessageEvent = msgEvent.copy()
      logger.info("newMessageEvent: {}", newMessageEvent)
      //first processing the message against intent table. If it is from choice, definitely intent is having that record otherwise we are sending it to keyword identification
      ...
      if (IntentID.nonEmpty) {
        ...
        logger.info("messageStoredInCache: {}", messageStoredInCache)
        msgEvent.msgDtls.messageTxt = messageStoredInCache
        msgEvent.msgDtls.msgTxtWithoutPunc = messageStoredInCache
        ...
        if (intentArray.contains(IntentID.get)) {
          ...
          msgEvent.msgDtls.messageTxt = payloadOrUserMsg
          msgEvent.msgDtls.msgTxtWithoutPunc = payloadOrUserMsg
          logger.info("newMessageEvent: {}", newMessageEvent)
          conversationPointerTbl.filter(x => x.SessionID === sessionID).delete
          //if user didn't select choice and enters text.
          userMsgForKwdIdentification(newMessageEvent, sessionID, None)
        }
      } else {
        logger.info("newMessageEvent: {}", newMessageEvent)
        conversationPointerTbl.filter(x => x.SessionID === sessionID).delete
      ...
      case ice: IntentNodesConfException =>
        logger.info("ice:{}", ice.getMessage)
  ...
  def userMsgForKwdIdentification(msgEvent: NlpReqCommonObj, sessionID: String, keyWordSet: Option[Set[String]])(implicit session: JdbcBackend#SessionDef) = { //db withSession { implicit session =>
    import profile.simple._
    ...
    try {
      ...
      if (intentsWithMoreThan1Tranact.length > 1) {
      ...
      } else if (intentArray.length >= 1 && intentArray.headOption.getOrElse(0L) != 0L) {

        logger.info("-----------------------intent found going to create nlp cache creation")
        ...
        entityValueIdentificationDao.entityValuesForConv(sessionID, msgEvent.msgDtls.messageTxt, intentArray.head)
        workflowDao.processWrkFlwPointer(sessionID, msgEvent)
