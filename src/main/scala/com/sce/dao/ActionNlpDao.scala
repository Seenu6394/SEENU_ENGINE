...
object ActionNlpDao extends NlpJsonSupport with NLGJsonSupport with DomainComponent with Profile {
  ...
  def rplceTextWithActionEntityValues(rawString: String, intentID: Long, replacers: List[TConversationCache]): String = { //= db withSession { implicit session =>
    ...
    try {
      ...
      val stringEntityIDs = entityPlaceHolders.findAllMatchIn(rawString).map(x => x.toString.substring(2, x.toString.length - 2)).toSet.toList
      val stringActionIDs = actionPlaceHolders.findAllMatchIn(rawString).map(x => x.toString.substring(2, x.toString.length - 2)).toSet.toList
  ...
  def insertActionLogs(actionId: Long, intentID: Long, entityID: Option[Long], webhookUrl: String, RequestBody: String, accessCode: String, result: String, userID: String, isSuccess: String)(implicit session: JdbcBackend#SessionDef) = { // = db withSession { implicit session =>
    ...
    (actionLog.map { x => ( x.ActionId, x.IntentID, x.EntityID, x.WebhookUrl, x.RequestBody, x.CallMethod, x.AccessCode, x.Result, x.Created, x.UserID, x.IsSuccess) } returning actionLog.map(_.ActionLogId)) += (
  ...
  def actionResponseprocessing(nlpDtls: NlpActionConsumerObj, actionResponse: String) = db withSession { implicit session =>
    ...
    try {
      val accessCode = iMSessionDao.getAccessCode(sender)
      ...
      logger.info("----------intent action flow--------------------")
        ...
        entityResponseHandlerForImage(actionResponse, nlpDtls.messageDtls, nlpDtls.identityDtls)
      }
    } catch {
  ...
  def actionValueForWrkflwConv(nlpDtls: NlpActionConsumerObj, actionResponse: String)(implicit session: JdbcBackend#SessionDef) ={
    import profile.simple._
    ...
    try {
      ...
      val accessCode = iMSessionDao.getAccessCode(senderID)
      var successNode = actionExtnRec.SuccessCode.getOrElse("")
      var errorNode = actionExtnRec.ErrorCode.getOrElse("")
