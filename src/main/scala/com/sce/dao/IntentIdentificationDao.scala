...
object IntentIdentificationDao extends NlpJsonSupport with DomainComponent with Profile {
    ...
    try {
      ...
      if (positiveIntentIds.length > 0) {
        ...
        if (intentArray.length == 0) {
          ...
          logger.info("-----------------------------------intentID found as: {}----------------------------------------", intentArray.headOption)
          ...
          logger.info("-----------------------------------list of intentIDs found as: {}----------------------------------------", intentArray)
          ...
          logger.info("ifIntentAlreadyChoosed: {}", ifIntentAlreadyChoosed)
          if (ifIntentAlreadyChoosed == 0L) {
            logger.info("------------------------------------inserting user rating log --------------------------------------")
  ...
  def insertKRLogs(iMSessionLogID: Long, intentID: Long, keywordRate: Int)(implicit session: JdbcBackend#SessionDef) = {
    import profile.simple._
    logger.info("--------------------------------------------------inserting kr logs------------------------------------------------------------------")
    ( KRLogTable.map(x => (x.IMSessionLogID, x.IntentID, x.KeywordRate) ) returning KRLogTable.map(_.KRLogID)) += ((iMSessionLogID, Some(intentID), Some(keywordRate)))
  }

  def insertUserRatingLogs(userID: String, iMSessionLogID: Long, keywords: String, intentID: Option[Long], sessionID: String)(implicit session: JdbcBackend#SessionDef): Long = {
    import profile.simple._

    val id = (userRating.map { x => (x.IMSessionLogID, x.Keywords, x.IntentID, x.UserID, x.SessionID) } returning userRating.map(_.RatingID)) += (iMSessionLogID, keywords, intentID, userID, Some(sessionID))
    logger.info("userRating log: {}", id)
    id
