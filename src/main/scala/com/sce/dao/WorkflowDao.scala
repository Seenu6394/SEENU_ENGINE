...
object WorkflowDao extends DomainComponent with Profile with NlpJsonSupport {
  ...
  def rplceTextWithActionEntityValuesForCond(rawString: String, intentID: Long, replacers: List[TConversationCache], sessionID: String)(implicit session: JdbcBackend#SessionDef): String = {
    ...
    try {
      ...
      val stringEntityIDs = entityPlaceHolders.findAllMatchIn(rawString).map(x => x.toString.substring(2, x.toString.length - 2).toLong).toSet.toList
      val stringActionIDs = actionPlaceHolders.findAllMatchIn(rawString).map(x => x.toString.substring(2, x.toString.length - 2).toLong).toSet.toList
      ...
      if (stringEntityIDs.length > 0 || stringActionIDs.length > 0) {
        ...
        logger.info("before lowercase of modifiedString: {}", modifiedString)
        modifiedString = modifiedString.toLowerCase()
        logger.info("after lowercase of modifiedString: {}", modifiedString)
      }
    } catch {
