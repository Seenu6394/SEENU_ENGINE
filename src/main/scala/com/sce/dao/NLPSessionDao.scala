...
object NLPSessionDao extends DomainComponent with Profile with NlpJsonSupport{
  ...
  def nlpCacheCreation(SessionID: String, IntentID: Long):Unit = db withSession { implicit session =>
    ...
    try {
      ...
      if (wrkFlowSeqRecords.nonEmpty) {
        ...
        logger.info("wrkFlowSeqRecords.length: {}", wrkFlowSeqRecords.length)
        ...
        if (wrkflwSeqID.nonEmpty) {
          ...
          throw new IntentNodesConfException("Error in workflowSeq Configuration")
        }
      ...
      case scs:IntentNodesConfException =>
        throw scs
